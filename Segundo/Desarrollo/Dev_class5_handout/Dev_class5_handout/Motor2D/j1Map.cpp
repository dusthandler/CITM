...
bool j1Map::LoadLayer(pugi::xml_node& node, MapLayer* layer)
{
	...
	layer->heigth = node.attribute("height").as_uint();
	layer->width = node.attribute("width").as_uint();

	layer->data = new uint[layer->width * layer->heigth];
	memset(layer->data, 0u, layer->width * layer->heigth);
	/*for (pugi::xml_node tileset = map_file.child("map").child("tileset"); tileset; tileset = tileset.next_sibling("tileset"))*/


	pugi::xml_node nodeid = node.child("data").child("tile");
	for (uint i=0;i<(layer->width * layer->heigth);i++)
	{
		layer->data[i] = nodeid.attribute("gid").as_uint();
		nodeid = nodeid.next_sibling("tile");
	}
